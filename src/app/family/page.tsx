...
async function loginAction(formData: FormData) {
  ...
  const expected = getEnvPassword('family');
  if (expected && password === expected) {
    await setAuthCookie('family');
    redirect('/family');
  }
  redirect('/family?error=1');
}

async function logoutAction() {
  'use server';
  await clearAuthCookie('family');
  redirect('/family');
}
...
export default async function FamilyPage({
  ...
  const auth = await isAuthorized('family');
    ...
    return (
      <main className="w-full px-6 sm:px-12 lg:px-60 py-12">
        ...
        <header className="mb-10">
          ...
          <p className="text-[var(--muted)] mt-2 text-sm">가족 전용 콘텐츠 — 비밀번호 입력</p>
        </header>
  ...
  const items = getAllContent().filter(c => c.visibility === 'family');
